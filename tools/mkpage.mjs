...
for (const part of md.split(/^## /m).slice(1)) {
  ...
  const m = head.match(/^(c\d\.jpg)\s*·\s*(.+)$/);
  if (!m) continue;
  const [, file, title] = m;
...
<title>פרומפטים למטריצת הסיכון</title>
...
<div class="wrap">
<header class="top">
  <p class="eyebrow">תחיקת הבנייה · שיעור 24 · הערכת סיכונים</p>
  <h1>פרומפטים למטריצת הסיכון</h1>
  <p class="sub">${shots.length} תמונות, אחת לכל תאונה. התמונה מראה את המפגע לפני האירוע, לא את התוצאה. כל כפתור מעתיק את בלוק הסגנון ואת הפרומפט יחד.</p>
  <div class="tally">
...
<div class="rule">
  ...
  <ol class="steps">
    ...
    <li>שומרים בשם שבכרטיס (<b>c1.jpg</b> וכו') ושולחים אחת-אחת.</li>
  </ol>
</div>
